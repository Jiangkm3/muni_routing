...
shape/service pattern is weighted by its share of scheduled trip occurrences in
the current feed. Street percentages are computed within each pattern first,
then averaged by those trip shares. This means a longer variant does not gain
extra influence merely because it is longer; an 80%/20% split of scheduled runs
contributes 80%/20% to the final route result.

You can instead use --aggregation canonical to select the most-used shape in
each direction and treat each selected direction once. This is often more
useful if you want a "typical physical route" rather than a service-weighted
measure.
...
-----
    ...
    python muni_route_street_analysis_normalized_cablecar_named_fallback_tunnels.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Map-match SFMTA Muni GTFS route shapes to OSM streets."
    )
    p.add_argument(
        "--api-key",
    ...
    p.add_argument(
        "--output",
        default="muni_route_streets_with_cablecar_pathmatch_whitelisted_tunnels_row.csv",
        help="Output CSV path (default: muni_route_streets_with_cablecar_pathmatch_whitelisted_tunnels_row.csv).",
    )
    p.add_argument(
        "--route-types",
        default="0,3,5",
...
def load_gtfs(gtfs_zip: Path, route_types: set[int]):
    ...
    trips = normalize_id_columns(trips, ["route_id", "service_id", "trip_id", "shape_id"])
    ...
    return routes, trips, shapes, stops


def choose_shape_weights(trips: pd.DataFrame, aggregation: str) -> pd.DataFrame:
    """Return route_id, shape_id, and a normalized service-pattern weight.

    In scheduled mode, shape_weight is the share of scheduled trip occurrences
    represented by that shape within the route. A GTFS trip that runs on 20
    service dates contributes 20 occurrences. The weights for a route sum to 1.
    """
    usage = (
    ...
    if aggregation == "scheduled":
        selected = (
            usage.groupby(["route_id", "shape_id"], as_index=False)["scheduled_trip_occurrences"]
            .sum()
        )
        totals = selected.groupby("route_id")["scheduled_trip_occurrences"].transform("sum")
        selected["shape_weight"] = selected["scheduled_trip_occurrences"] / totals
        return selected[["route_id", "shape_id", "shape_weight", "scheduled_trip_occurrences"]]

    # Canonical: select the highest scheduled-use shape in each direction. Give
    # each selected direction equal influence in the combined route result.
    ...
    counts = canonical.groupby("route_id")["shape_id"].transform("count")
    canonical["shape_weight"] = 1.0 / counts
    return canonical[["route_id", "shape_id", "shape_weight", "scheduled_trip_occurrences"]]
...
def analyze(
    ...
    accum_share: dict[tuple[str, str], float] = defaultdict(float)

    # Length is the service-weighted mean meters traveled on each street/category
    # per scheduled trip. It is intentionally tracked separately because
    # percentage weighting normalizes each pattern first.
    accum_length_m: dict[tuple[str, str], float] = defaultdict(float)
    ...
    records = [
        {
            ...
            "street_length_m": accum_length_m[(route_id, street)],
        }
        for (route_id, street), share in accum_share.items()
    ]
    result = pd.DataFrame(records)
    if result.empty:
        raise RuntimeError("No route/street matches were produced.")

    result = coalesce_normalized_streets(result, ["route_id"])

    totals = result.groupby("route_id")["service_weighted_share"].sum().rename("route_share_total")
    result = result.merge(totals, on="route_id", how="left")
    result["street_share_pct"] = (
        100.0 * result["service_weighted_share"] / result["route_share_total"]
    )
...
def main() -> int:
    ...
    routes, trips, shapes, stops = load_gtfs(gtfs_zip, route_types)
    if routes.empty:
    ...
    result = result.sort_values(
        ["route_short_name", "street_share_pct"], ascending=[True, False], na_position="last"
    )
